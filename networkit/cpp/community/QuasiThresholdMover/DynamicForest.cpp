...
bool DynamicForest::pathsValid() {
    ...
    for (pid freePlace : freeList) {
        ...
        assert(paths[freePlace].pathNodes.empty());
        assert(paths[freePlace].childPaths.empty());
...
#ifndef NDEBUG
    assert(i != none);
    assert(paths[i].childPaths.empty());
    for (pid sp = 0; sp < paths.size(); sp++) {
