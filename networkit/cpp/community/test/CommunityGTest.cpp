...
TEST_F(CommunityGTest, testWeightedMatrixSubtreeMove) {
  Aux::Random::setSeed(37, false);
  std::vector<std::vector<int64_t>>  editCostMatrix;
  editCostMatrix.resize(34);
  for(count i = 0; i< editCostMatrix.size(); i++){
...
TEST_F(CommunityGTest, testWeightedMatrixSubtreeMoveRnd) {
  Aux::Random::setSeed(37, false);
  std::vector<std::vector<int64_t>>  editCostMatrix;
  editCostMatrix.resize(34);
  for(count i = 0; i< editCostMatrix.size(); i++){
  ...
  QuasiThresholdMoving::QuasiThresholdEditingLocalMover mover(karate, QuasiThresholdMoving::QuasiThresholdEditingLocalMover::ASC_DEGREE_INSERT, 20, true, true, true, 4UL, true, 5, 2, editCostMatrix);
  mover.run();
  Graph Q = mover.getQuasiThresholdGraph();
  count used = mover.getNumberOfEdits();
  count usedWeight = mover.getWeightOfEdits();
