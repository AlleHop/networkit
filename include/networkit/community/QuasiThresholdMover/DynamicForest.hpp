...
public:
    ...
    std::vector<node> children(node u) const {
        ...
        forChildrenOf(u, [&](node c) { result.emplace_back(c); });
    ...
    node nextChild(node child, node p) const {
        ...
        if (isLowerEnd(p)) {
            ...
            if (nextChildPath != path(p))
                return paths[nextChildPath].upperEnd();
        }
        return p;
    };
